# 📒 Référentiel central — Groupe MDM

> Source de vérité des **faits stables**. Hermès doit le consulter **avant** de demander ou de calculer une donnée.
> Règle d'or : on **ne devine pas**. Toute valeur incertaine est marquée **[À VÉRIFIER]** (à confirmer sur source officielle) ou **[À COMPLÉTER]** (donnée manquante).
> Mettre à jour ce fichier dès qu'une info est confirmée par Michel.

## 1. Entités juridiques

| Société | Forme | SIREN | Siège | Activité |
|---|---|---|---|---|
| **MDMProd** | SAS | 825 335 003 (RCS Paris) | 6 rue d'Armaillé, 75017 Paris | Production audiovisuelle, spectacle vivant, événements, coproduction, production déléguée |
| **Neshama Music** | SARL | 807 857 719 (RCS Paris) | 61 rue de Lyon, 75012 Paris | Activités artistiques/musicales/événementielles — **titulaire licence entrepreneur de spectacles** |
| **SpectaGestion / PaieIntermittents.com** | Marque de MDMProd | — | — | Paie des intermittents + déclarations (DPAE → DSN) ; **le client reste l'employeur** |

- N° licence entrepreneur de spectacles (Neshama) : **[À COMPLÉTER]**
- TVA intracom MDMProd / Neshama : **[À COMPLÉTER]**

## 2. Identité & marques
- **Michel Méquiès** : identité professionnelle (jamais « Michel David Méquiès » sauf mention légale d'état civil).
- **David Méquiès** : nom de scène, **réservé** au site chanteursynagogue.art (pianiste & chanteur de cérémonie, « depuis 1987 » / « plus de 35 ans »).
- Sites : mdmprod.fr · paieintermittents.com · chanteursynagogue.art

## 3. Coordonnées (à confirmer/compléter)
- Tél pro MDM : 01 82 28 99 60 · 06 61 40 51 40  **[À VÉRIFIER]**
- E-mail pro : michel@mdmprod.fr · contact@mdmprod.fr  **[À VÉRIFIER]**
- Comptabilité : Pennylane (fournisseurs : `neshama-music-...@suppliers.pennylane.com`)  **[À VÉRIFIER]**

## 4. Tarifs SpectaGestion
- **18 € HT / bulletin** + **14,90 € HT / mois**.
- Au-delà de **50 bulletins/mois** : tarif sur mesure.

## 5. Référentiel paie intermittents — ⚠️ à revérifier chaque année sur source officielle
*(Ne jamais traiter un taux comme acquis. Confirmer avant tout document définitif.)*
- **CDDU** = contrat à durée déterminée d'usage.
- Seuil d'ouverture de droits intermittents : **507 h / 12 mois** [À VÉRIFIER].
- Annexes France Travail : **8** (techniciens), **10** (artistes).
- Organismes : URSSAF, France Travail, **Audiens** (retraite/prévoyance), **Congés Spectacles**, **AFDAS**, **CMB** (médecine du travail).
- **DFS** (déduction forfaitaire spécifique) : musiciens **16 %** / artistes dramatiques-lyriques **18 %** [À VÉRIFIER chaque année] ; jamais sur chômage, AGS, Congés Spectacles, CSG/CRDS ; **accord du salarié requis** ; plafond annuel [À VÉRIFIER].
- Sources officielles : urssaf.fr · francetravail.fr (annexes 8/10) · audiens.org · conges-spectacles.com · afdas.com · legifrance.gouv.fr · net-entreprises.fr

## 6. Juridique / contrats
- **Pas de portage salarial** (interdit dans le spectacle).
- TVA billetterie spectacle vivant : ex. **5,5 %** [À VÉRIFIER selon cas].
- Indemnité forfaitaire de recouvrement : **40 €** (art. L441-10 C. com.) [À VÉRIFIER].
- Droits : SACEM / SACD selon le cas.

## 7. Projets en cours (synthèse — détail ailleurs)
- **Immobilier Touques** : offre prêt SG ≈ 387 000 € / 3,10 % / 228 mois → **2 383 €/mois** [À VÉRIFIER]. Courtier : Pretto (Alban de Villele).
- **Trésorerie** (à actualiser via Qonto) : voir agent /tresorerie.

## 8. Fonctions & rôles de Michel
- **Conseiller prud'homal** au **CPH de Paris** (Encadrement, Ch. 8). ⚠️ Dans cette fonction, Michel est **juge**, pas partie : indépendance, **secret du délibéré**, pas de mandat impératif. → ressource dédiée : dossier `prudhommes/` (sources officielles) + sous-agent `conseiller-prudhommes`. **Ne pas confondre** avec l'agent /juridique (où Michel serait partie à un litige).
- Dirigeant du Groupe MDM (MDMProd, Neshama Music, SpectaGestion).

## 9. Contacts & partenaires (à confirmer/compléter)
- **Courtier crédit** : Pretto — Alban de Villele (alban@pretto.fr) [À VÉRIFIER].
- **Banques sollicitées (prêt Touques)** : Société Générale, CCF ; **Fortuneo a refusé** (08/06/2026).
- **Comptabilité** : Pennylane.
- **Protection juridique** : Juridica (dossier MEQUIES / SCP Billaudel) [À VÉRIFIER].
- **Expert-comptable** : [À COMPLÉTER].

## 10. Véhicule
- **Audi e-tron GT** — immat. **GX-370-ZS**, 1ʳᵉ immatriculation **29/06/2024**, ~26 000 km.
- Extension de garantie : offre réf. **2ba249** via VWFS (vwfs-garantie.fr) — dossier en cours [À SUIVRE].

---
*Dernière mise à jour : à dater à chaque modification. Toute donnée [À VÉRIFIER] reste à confirmer avant usage officiel.*
