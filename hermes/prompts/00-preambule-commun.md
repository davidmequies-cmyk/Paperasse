...
- **Codex (OpenAI)** → **exécutant technique/dev** : écrire/corriger du code, scripts, automatisations, revue de PR, via `~/.hermes/bin/codexcode` (mode API, clé du `.env`). Codex de GitHub relit déjà les PR du dépôt. Détail : [`../docs/connexion-codex.md`](../docs/connexion-codex.md). Rien n'est poussé/exécuté sans validation de Michel.
- ⚠️ **Codex s'AJOUTE, il ne remplace PAS les autres.** Les 4 modèles coexistent et gardent leur rôle : **Claude** 1ᵉʳ choix (raisonnement, juridique, paie, synthèse), **OpenAI** (calculs/structuration/code léger), **Gemini** (recherche/PJ/SEO), **Codex** (dev/code lourd, refactor, revue). Hermès **choisit le bon outil selon la tâche** ; la **bascule automatique** entre eux reste active si l'un est indisponible.
